...
class HandEvaluator:
    ...
    def evaluate_hand(player_cards: list[Card], community_cards: list[Card]) -> int:
        ...
        return -1  # High Card
    @staticmethod
    def hand_comparator(hand1: list[Card], hand2: list[Card], public_cards: list[Card]) -> int:
        ...
        if HandEvaluator.is_royal_flush(cards1):
            ...
            if HandEvaluator.get_straight_flush_high_card(cards1) > HandEvaluator.get_straight_flush_high_card(cards2):
                return 1
            elif HandEvaluator.get_straight_flush_high_card(cards1) < HandEvaluator.get_straight_flush_high_card(cards2):
                return -1
            return 0
        elif HandEvaluator.is_four_of_a_kind(cards1):
            if HandEvaluator.get_four_of_a_kind_rank(cards1) > HandEvaluator.get_four_of_a_kind_rank(cards2):
                return 1
            elif HandEvaluator.get_four_of_a_kind_rank(cards1) < HandEvaluator.get_four_of_a_kind_rank(cards2):
                return -1
            elif max(card.rank for card in cards1 if card.rank != HandEvaluator.get_four_of_a_kind_rank(cards1)) > max(card.rank for card in cards2 if card.rank != HandEvaluator.get_four_of_a_kind_rank(cards2)):
                return 1
            elif max(card.rank for card in cards1 if card.rank != HandEvaluator.get_four_of_a_kind_rank(cards1)) < max(card.rank for card in cards2 if card.rank != HandEvaluator.get_four_of_a_kind_rank(cards2)):
                return -1
            return 0
        elif HandEvaluator.is_full_house(cards1):
            if HandEvaluator.get_three_of_a_kind_rank(cards1) > HandEvaluator.get_three_of_a_kind_rank(cards2):
                return 1
            elif HandEvaluator.get_three_of_a_kind_rank(cards1) < HandEvaluator.get_three_of_a_kind_rank(cards2):
                return -1
            elif HandEvaluator.get_full_house_pair_rank(cards1) > HandEvaluator.get_full_house_pair_rank(cards2):
                return 1
            elif HandEvaluator.get_full_house_pair_rank(cards1) < HandEvaluator.get_full_house_pair_rank(cards2):
                return -1
            return 0
        elif HandEvaluator.is_flush(cards1):
            if HandEvaluator.get_flush_ranks(cards1) > HandEvaluator.get_flush_ranks(cards2):
                return 1
            elif HandEvaluator.get_flush_ranks(cards1) < HandEvaluator.get_flush_ranks(cards2):
                return -1
            return 0
        elif HandEvaluator.is_straight(cards1):
            if HandEvaluator.get_straight_high_card(cards1) > HandEvaluator.get_straight_high_card(cards2):
                return 1
            elif HandEvaluator.get_straight_high_card(cards1) < HandEvaluator.get_straight_high_card(cards2):
                return -1
            return 0
            ...
            if HandEvaluator.get_three_of_a_kind_rank(cards1) > HandEvaluator.get_three_of_a_kind_rank(cards2):
                ...
                return -1
            cards1_kickers = sorted((card.rank for card in cards1 if card.rank != HandEvaluator.get_three_of_a_kind_rank(cards1)), reverse=True)[:2]
            cards2_kickers = sorted((card.rank for card in cards2 if card.rank != HandEvaluator.get_three_of_a_kind_rank(cards2)), reverse=True)[:2]
            ...
            if HandEvaluator.get_two_pair_ranks(cards1) > HandEvaluator.get_two_pair_ranks(cards2):
                return 1
            elif HandEvaluator.get_two_pair_ranks(cards1) < HandEvaluator.get_two_pair_ranks(cards2):
                return -1
            cards1_kicker = max(card.rank for card in cards1 if card.rank not in HandEvaluator.get_two_pair_ranks(cards1))
            cards2_kicker = max(card.rank for card in cards2 if card.rank not in HandEvaluator.get_two_pair_ranks(cards2))
            ...
            if HandEvaluator.get_one_pair_rank(cards1) > HandEvaluator.get_one_pair_rank(cards2):
                return 1
            elif HandEvaluator.get_one_pair_rank(cards1) < HandEvaluator.get_one_pair_rank(cards2):
                return -1
            cards1_kickers = sorted((card.rank for card in cards1 if card.rank != HandEvaluator.get_one_pair_rank(cards1)), reverse=True)[:3]
            cards2_kickers = sorted((card.rank for card in cards2 if card.rank != HandEvaluator.get_one_pair_rank(cards2)), reverse=True)[:3]
            if cards1_kickers > cards2_kickers:
                return 1
            elif cards1_kickers < cards2_kickers:
                return -1
    ...
    def get_flush_ranks(cards: list[Card]) -> list[int]:
        ...
        for suit, count in suit_counts.items():
            if count >= 5:
                return sorted((card.rank for card in cards if card.suit == suit), reverse=True)[:5]
        return []
    @staticmethod
    ...
    def get_full_house_pair_rank(cards: list[Card]) -> int:
        ...
        pair_ranks = [rank for rank, count in rank_counts.items() if rank != trip_rank and count >= 2]
        if pair_ranks:
